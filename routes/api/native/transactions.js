...
module.exports = (api) => {  
    ...
    for (let i = 0; i < array.length; i++)
    {
        promiseArray.push(
          new Promise((resolve, reject) => {
            api.native.callDaemon(coin, 'gettransaction', [array[i].txid], token)
            .then((__json) => {
              resolve(__json);
            })
          }
  ...
  api.native.get_transactions = (coin, token, includePrivate, maxPubTransactions = 200) => {
    let privateAddresses = []
    let transactions = []

    return new Promise((resolve, reject) => {
      let transactionPromises = [api.native.callDaemon(coin, 'listtransactions', ['*', maxPubTransactions], token)]
      if (includePrivate) transactionPromises.push(api.native.callDaemon(coin, 'z_listaddresses', [], token))
      ...
      .then((jsonResults) => {
        ...
        return Promise.all(privateAddresses.map((address) => {
          return api.native.callDaemon(coin, 'z_listreceivedbyaddress', [address], token)
        }))
      })
      .then((receivedByAddressList) => {
        const privateTxs = receivedByAddressList.map((receivedByAddressArray, addressIndex) => {
          return receivedByAddressArray.map((privTx) => {
            return {...privTx, address: privateAddresses[addressIndex], category: 'receive'}
          })
        }).flat()
        
        return privateTxs.length > 0 ? getZTransactionGroups(coin, privateTxs, [privateTxs], token) : [[]]
      })
  ...
  api.post('/native/get_transactions', (req, res, next) => {
    ...
    api.native.get_transactions(coin, token, includePrivate, maxPubTransactions)
    .then((transactions) => {
      const retObj = {
        msg: 'success',
        result: transactions,
